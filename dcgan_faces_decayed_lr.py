...
out_dir = "./dcgan_faces_montage_outputs/dcgan_faces_decreasing_lr"

# method to save print messages to txt file
losses_file = "./dcgan_faces_text_outputs/dcgan_faces_decreasing_lr_losses.txt"
printfile = "./dcgan_faces_text_outputs/dcgan_faces_decreasing_lr.txt"
...
# resize the images if not already done
if not os.path.exists(resized_img_dir):
    txtprint("Resizing Images")
...
with tf.control_dependencies(update_ops):
    ...
    optimizer_d = tf.train.RMSPropOptimizer(learning_rate=learning_rate).minimize(loss_d + d_reg, var_list=vars_d, global_step=global_step)
    # minimize the generator loss plus the l-2 regularization of generator's weights
    optimizer_g = tf.train.RMSPropOptimizer(learning_rate=learning_rate*2).minimize(loss_g + g_reg, var_list=vars_g, global_step=global_step)
...
for i in range(num_iterations):
    ...
    if not i % 10:
        ...
        txtprint('D loss: {:.4}'.format(d_ls))
        txtprint('G_loss: {:.4}'.format(g_ls))
        txtprint(' ')
        if not train_g:
            txtprint("not training generator")
        if not train_d:
            txtprint("not training discriminator")
