...
out_dir = "./dcgan_faces_montage_outputs/dcgan_faces_change_optimizers_.005_lr"

# method to save print messages to txt file
losses_file = "./dcgan_faces_text_outputs/dcgan_faces_change_optimizers_.005_lr_losses.txt"
printfile = "./dcgan_faces_text_outputs/dcgan_faces_change_optimizers_.005_lr.txt"
...
with tf.control_dependencies(update_ops):
    ...
    optimizer_d = tf.train.GradientDescentOptimizer(.005).minimize(loss_d + d_reg, var_list=vars_d)
    # minimize the generator loss plus the l-2 regularization of generator's weights
    # use ADAM for generator
    optimizer_g = tf.train.AdamOptimizer(.005).minimize(loss_g + g_reg, var_list=vars_g)
